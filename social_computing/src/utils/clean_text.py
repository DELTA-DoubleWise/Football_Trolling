...
def clean_all(df, col_name):
    ...
    for word in manager_names:
        df[col_name] = df[col_name].replace((" " + word + " "), " managers ", regex=True)
    for word in stadium_names:
        df[col_name] = df[col_name].replace((" " + word + " "), " stadium ", regex=True)
    for word in player_names:
        df[col_name] = df[col_name].replace((" " + word + " "), " player ", regex=True)
